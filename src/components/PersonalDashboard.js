...
const PersonalDashboard = ({ user }) => {
  ...
  const handleSubmitActividad = async (e) => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/api/university/actividades`, formDataToSend, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  const handleSubmitEdicion = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_URL}/api/university/actividades/${actividadAEditar.id}`,
        formDataToSend,
      ...
      toast.error(error.response?.data?.error || 'Error al actualizar la actividad');
    } finally {
            ...
            <form onSubmit={handleSubmitEdicion}>
              ...
              <div className="form-grid dates-grid">
                <div className="form-group">
                  ...
                  <input type="date" name="fecha_inicio" value={editForm.fecha_inicio} onChange={handleEditChange} required />
                </div>
                <div className="form-group">
                  <label>Fecha de Fin (Opcional)</label>
                  <input type="date" name="fecha_fin" value={editForm.fecha_fin} onChange={handleEditChange} min={editForm.fecha_inicio || ''} />
                </div>
              </div>
                ...
                <div className="form-group">
                  ...
                  <div className="image-upload-area">
                    <input
                      ...
                      disabled={(actividadAEditar.imagenes?.length || 0) + editForm.imagenesNuevas.length >= 5}
                      style={{ display: 'none' }}
                    />
                    <label
                      htmlFor="imagenes-edit"
                      className={`image-upload-label ${isDragOverEdit ? 'drag-over' : ''}`}
                      onDragOver={(e) => { e.preventDefault(); setIsDragOverEdit(true); }}
                      onDragLeave={(e) => { e.preventDefault(); setIsDragOverEdit(false); }}
                      onDrop={(e) => { e.preventDefault(); setIsDragOverEdit(false); handleEditImageUpload(e.dataTransfer.files); }}
                    >
                      <div className="upload-icon">📷</div>
                      <div className="upload-text">Agregar imágenes</div>
                      <div className="upload-hint">Haz clic o arrastra aquí</div>
                      <div className="upload-limit">Máx. 5 en total • 5MB c/u</div>
                    </label>
                  </div>
                  {editForm.imagenesNuevas.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', marginTop: '10px' }}>
                      {editForm.imagenesNuevas.map((img, index) => (
                        <div key={index} style={{ position: 'relative', width: '100px', height: '100px' }}>
                          <img src={img.preview} alt={img.nombre} style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: '8px', border: '2px solid #28a745' }} />
                          <button
                            type="button"
                            onClick={() => removeEditImageNueva(index)}
                            style={{
                              position: 'absolute', top: '-6px', right: '-6px',
                              background: '#dc3545', color: 'white', border: 'none',
                              borderRadius: '50%', width: '22px', height: '22px',
        ...
        <div className="form-modal">
          <div className="form-modal-content large-modal">
            ...
            <form onSubmit={handleSubmitActividad}>
              <div className="form-group">
                <label>Título de la Actividad *</label>
                <input type="text" name="titulo" value={formData.titulo} onChange={handleChange} placeholder="Ej: Jornada de capacitación técnica" required autoFocus />
              </div>
              <div className="form-group">
                <label>Tipo de Actividad *</label>
                <input type="text" name="tipo_actividad" value={formData.tipo_actividad} onChange={handleChange} placeholder="Ej: Taller, Conferencia, Reunión de trabajo, Seminario, etc." required maxLength="100" />
                <small className="form-hint">
                  Describe qué tipo de actividad es (máximo 100 caracteres)
                  <span className="word-counter" style={{ display: 'block', marginTop: '5px' }}>{formData.tipo_actividad.length} / 100 caracteres</span>
                </small>
              </div>
              <div className="form-group">
              ...
              <div className="form-grid dates-grid">
                <div className="form-group">
                  <label>Fecha de Inicio *</label>
                  <input type="date" name="fecha_inicio" value={formData.fecha_inicio} onChange={handleChange} required min={getMinDate()} max={getMaxDate()} />
                  <small className="form-hint">Puedes seleccionar desde {getMinDate()} hasta {getMaxDate()}</small>
                </div>
                <div className="form-group">
                  <label>Fecha de Fin (Opcional)</label>
                  <input type="date" name="fecha_fin" value={formData.fecha_fin} onChange={handleChange} min={formData.fecha_inicio || getMinDate()} max={getMaxDate()} />
              ...
              <div className="form-group">
                ...
                <div className="image-upload-container">
                  <div className="image-upload-area">
                    <input type="file" id="image-upload" accept="image/*" multiple onChange={(e) => handleImageUpload(e.target.files)} disabled={uploadingImages || formData.imagenes.length >= 5} style={{ display: 'none' }} />
                    <label htmlFor="image-upload" className={`image-upload-label ${isDragOver ? 'drag-over' : ''} ${uploadingImages ? 'uploading' : ''}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
                      <div className="upload-icon">📷</div>
                      <div className="upload-text">{isDragOver ? 'Suelta las imágenes aquí' : 'Subir imágenes del evento'}</div>
                      <div className="upload-hint">{isDragOver ? 'Suelta para cargar' : 'Haz clic o arrastra imágenes aquí'}</div>
                      <div className="upload-limit">Máximo 5 imágenes • 5MB cada una • Formatos: JPG, PNG, GIF</div>
                    </label>
                        ...
                        <div className="preview-grid">
                          {formData.imagenes.map((img, index) => (
                            <div key={index} className="preview-item">
                              <img src={img.preview} alt={`Vista previa ${index + 1}`} className="preview-image" />
                              <button type="button" className="remove-image-btn" onClick={() => removeImage(index)} title="Eliminar imagen">×</button>
                              <div className="image-info"><small>{img.nombre}</small><small>{(img.tamano / 1024).toFixed(1)} KB</small></div>
                            </div>
                          ))}
                        </div>
                      </div>
