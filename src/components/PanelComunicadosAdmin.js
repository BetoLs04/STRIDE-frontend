...
const PanelComunicadosAdmin = ({ admin, onClose }) => {
  ...
  const fetchComunicadosAdmin = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/university/comunicados-admin');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        ...
        await axios.put(`/api/university/comunicados/${editId}`, comunicadoData);
        toast.success('Comunicado actualizado exitosamente');
      } else {
        // Crear nuevo comunicado
        await axios.post('/api/university/comunicados', comunicadoData);
  ...
  const handleDelete = async (id, titulo) => {
    ...
    try {
      await axios.delete(`/api/university/comunicados/${id}`);
