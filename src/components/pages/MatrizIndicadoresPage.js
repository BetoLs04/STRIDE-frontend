...
const MatrizIndicadoresPage = ({ user }) => {
  ...
  const saveFila = async (fila, valores) => {
    const res = await api.put(`/api/university/matriz-filas/${fila.id}`, { valores });
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [secRes, encRes, colRes, filasRes] = await Promise.all([
        ...
        api.get('/api/university/matriz-encabezado'),
        api.get('/api/university/matriz-columnas'),
        api.get(`/api/university/matriz-filas/${seccionId}`)
      ]);
      ...
      handleApiError(error, 'Error al cargar datos de la matriz');
    } finally {
      setLoading(false);
    }
  }, [seccionId]);
  ...
  const handleUnidadChange = async (fila, value) => {
    ...
    try {
      ...
      handleApiError(error, 'Error al guardar unidad de medida');
    }
  };
  ...
  const handleAddFila = async () => {
    setAdding(true);
    try {
      const res = await api.post('/api/university/matriz-filas', {
  ...
  const handleDeleteFila = async (fila) => {
    ...
    try {
      await api.delete(`/api/university/matriz-filas/${fila.id}`);
