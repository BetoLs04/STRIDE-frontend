...
import { MESSAGES, STATUS, USER_TYPES } from '../../constants/index';
...
const SuperAdminActividades = ({ admin }) => {
  ...
  useEffect(() => {
    ...
    if (admin.tipo !== USER_TYPES.SUPERADMIN) {
      toast.error(MESSAGES.ACCESO_NO_AUTORIZADO);
      navigate(ROUTES.LOGIN);
      return;
    }
  ...
  const getImageUrl = (url) => {
    ...
    if (url.startsWith('http://') || url.startsWith('https://')) {
      ...
      if (match) return `https://api1.strideutmat.com${match[1]}`;
      return url;
    }
    return `https://api1.strideutmat.com${url}`;
  };
  ...
  const fetchTodasActividades = async () => {
    try {
      ...
      const direccionesRes = await api.get('/api/university/direcciones');
      ...
      for (const direccion of direccionesList) {
        try {
          ...
          const response = await api.get(`/api/university/actividades/direccion/${direccion.id}`);
          ...
          handleApiError(dirError, 'Error al cargar datos');
        }
      }
      ...
      if (todasActividades.length === 0) {
        ...
        toast.info(MESSAGES.SIN_ACTIVIDADES);
      }
    ...
    const actividadesEjemplo = [
      {
        ...
        estado: STATUS.ACTIVIDAD.COMPLETADA,
      ...
      },
      {
        ...
        estado: STATUS.ACTIVIDAD.EN_PROGRESO,
      ...
      },
      {
        ...
        creado_por_tipo: 'personal',
        estado: STATUS.ACTIVIDAD.PENDIENTE,
        ...
        {error ? (
          ...
          <div className="no-data">
            ...
            <div className="no-data-actions">
              <button className="btn btn-primary" onClick={() => navigate(ROUTES.ADMIN_DASHBOARD)}>
            ...
            <table className="tabla-direcciones-compacta">
              ...
              <tbody>
                {direcciones.map(direccion => {
                  ...
                  const completadas = actividadesDir.filter(a => a.estado === STATUS.ACTIVIDAD.COMPLETADA).length;
                  const enProgreso = actividadesDir.filter(a => a.estado === STATUS.ACTIVIDAD.EN_PROGRESO).length;
                  const pendientes = actividadesDir.filter(a => a.estado === STATUS.ACTIVIDAD.PENDIENTE).length;
          ...
          <div className="resumen-creadores-mejorado">
            ...
            <div className="creadores-resumen-grid">
              {tiposActividad.map((tipo, index) => {
                ...
                const completadas = actividadesDeEsteTipo.filter(a => a.estado === STATUS.ACTIVIDAD.COMPLETADA).length;
                const enProgreso = actividadesDeEsteTipo.filter(a => a.estado === STATUS.ACTIVIDAD.EN_PROGRESO).length;
                const pendientes = actividadesDeEsteTipo.filter(a => a.estado === STATUS.ACTIVIDAD.PENDIENTE).length;
