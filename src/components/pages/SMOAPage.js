...
const SMOAPage = ({ user }) => {
  ...
  const saveFila = async (fila, valores) => {
    const res = await api.put(`/api/university/smoa-filas/${fila.id}`, { valores });
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [encRes, colRes, filasRes, permisosRes] = await Promise.all([
        api.get('/api/university/smoa-encabezado'),
        api.get('/api/university/smoa-columnas'),
        api.get('/api/university/smoa-filas'),
        esSuperAdmin ? Promise.resolve({ data: { data: {} } }) : api.get('/api/university/smoa-permisos-pptx')
      ]);
  ...
  const handleAddFila = async () => {
    setAdding(true);
    try {
      const res = await api.post('/api/university/smoa-filas', { valores: {} });
  ...
  const handleDeleteFila = async (fila) => {
    ...
    try {
      ...
      await api.delete(`/api/university/smoa-filas/${fila.id}`);
  ...
  const handleUploadPptx = async (fila, file) => {
    ...
    try {
      ...
      const res = await api.put(`/api/university/smoa-filas/${fila.id}/pptx`, formData, {
  ...
  const handleEliminarPptx = async (fila) => {
    ...
    try {
      const res = await api.put(`/api/university/smoa-filas/${fila.id}/pptx`, { eliminar: 'true' });
      setFilas(prev => prev.map(f => f.id === fila.id ? res.data.data : f));
  ...
  const handleSaveLinkModal = async () => {
    ...
    try {
      ...
      handleApiError(error, 'Error al guardar enlace');
    } finally {
  ...
  const handleDeleteLink = async (fila, columna) => {
    ...
    try {
      ...
      handleApiError(error, 'Error al eliminar enlace');
    }
  };
  ...
  const handleUploadColFile = async (fila, columna, file) => {
    ...
    try {
      ...
      const res = await api.post(
        `/api/university/smoa-filas/${fila.id}/columna/${columna.id}/subir`,
        formData,
        { headers: { 'Content-Type': 'multipart/form-data' } }
      );
      ...
      handleApiError(error, 'Error al subir archivo');
    } finally {
  ...
  const handleDeleteColFile = async (fila, columna) => {
    ...
    try {
      const res = await api.delete(
        `/api/university/smoa-filas/${fila.id}/columna/${columna.id}/eliminar`
      );
      setFilas(prev => prev.map(f => f.id === fila.id ? res.data.data : f));
  ...
  const goBack = () => {
    if (esSuperAdmin) navigate(ROUTES.ADMIN_DASHBOARD, { state: { tab: 'smoa' } });
