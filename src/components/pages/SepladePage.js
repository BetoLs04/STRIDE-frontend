...
const SepladePage = ({ user }) => {
  ...
  const fetchHoja = useCallback(async () => {
    ...
    try {
      ...
      if (usuariosRes?.data?.success) {
        ...
        handleApiError(error, 'Error al cargar datos SEPLADE');
      } finally {
        setLoading(false);
      }
  }, [hojaId]);
  ...
  const handleSaveModal = async () => {
    ...
    try {
      if (modalField === 'valor') {
        await api.put(`/api/university/seplade-valores/${modalIndicadorId}`, {
          mes: modalMes,
          tipo: modalTipo,
          valor: modalValue
        });
        ...
        await api.put(`/api/university/seplade-indicadores/${modalIndicadorId}`, {
  ...
  const handleSaveNote = async () => {
    ...
    setNoteSaving(true);
    try {
      await api.put(`/api/university/seplade-notas/${noteIndicadorId}/${noteMes}`, {
        nota: noteModalValue
      });
      setNotas(prev => {
      ...
      handleApiError(error, 'Error al guardar valor');
    } finally {
      setNoteSaving(false);
    }
  };
  ...
  const handleAddIndicador = async () => {
    setAdding(true);
    try {
      const res = await api.post('/api/university/seplade-indicadores', { hoja_id: parseInt(hojaId) });
      ...
      handleApiError(error, 'Error al guardar valores');
    } finally {
      setAdding(false);
    }
  };
  ...
  const handleAsignarUsuario = async (indicadorId, usuarioId, usuarioTipo) => {
    ...
    try {
      await api.post(`/api/university/seplade-indicadores/${indicadorId}/usuarios`, {
  ...
  const handleQuitarUsuario = async (indicadorId, usuarioId, usuarioTipo) => {
    try {
      await api.delete(`/api/university/seplade-indicadores/${indicadorId}/usuarios/${usuarioId}/${usuarioTipo}`);
      ...
      handleApiError(error, 'Error al eliminar valor');
    }
  };
  ...
  const handleDeleteIndicador = async (indicadorId) => {
    ...
    try {
      await api.delete(`/api/university/seplade-indicadores/${indicadorId}`);
      ...
      handleApiError(error, 'Error al navegar');
    }
  };

  const goBack = () => {
    if (esSuperAdmin) navigate(ROUTES.ADMIN_DASHBOARD, { state: { tab: 'seplade' } });
    else if (esDirectivo) navigate(ROUTES.DIRECTIVO_DASHBOARD);
    else if (esPersonal) navigate(ROUTES.PERSONAL_DASHBOARD);
