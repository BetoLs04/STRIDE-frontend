...
const PersonalTareas = ({ user }) => {
  ...
  const cargarTareas = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/api/university/tareas/personal/${user.id}`);
  ...
  const calcularStats = (tareasList) => {
    ...
    tareasList.forEach(t => {
      if (t.asignacion_estado === STATUS.TAREA.PENDIENTE) {
      ...
      } else if (t.asignacion_estado === STATUS.TAREA.EN_PROGRESO) {
        nuevasStats.en_progreso++;
      } else if (t.asignacion_estado === STATUS.TAREA.COMPLETADA) {
  ...
  const handleSubmitResponder = async (e) => {
    ...
    try {
      ...
      const response = await api.post(
        `/api/university/tareas/completar/${tareaAResponder.asignacion_id}`,
        formData,
      ...
      handleApiError(error, 'Error al completar tarea');
    } finally {
      setSubiendo(false);
    }
  };
  ...
  const handleDeleteArchivo = async (archivoId, e) => {
    ...
    try {
      ...
      handleApiError(error, 'Error al cargar archivo');
    }
  };
  ...
  const getEstadoClase = (dias, estado) => {
    if (estado === STATUS.TAREA.COMPLETADA) return STATUS.TAREA.COMPLETADA;
  ...
  const tareasFiltradas = tareas.filter(t => {
    if (filtro === 'pendientes') return t.asignacion_estado === STATUS.TAREA.PENDIENTE || t.asignacion_estado === STATUS.TAREA.EN_PROGRESO;
    if (filtro === 'completadas') return t.asignacion_estado === STATUS.TAREA.COMPLETADA;
    return true;
  });
        ...
        <div className="tareas-lista">
          {tareasFiltradas.length === 0 ? (
            ...
            tareasFiltradas.map(tarea => {
              ...
              const puedeResponder = tarea.asignacion_estado !== STATUS.TAREA.COMPLETADA;
              return (
                <div key={tarea.id} className={`tarea-item ${estadoClase} ${tarea.asignacion_estado}`}>
                  ...
                  <div className="tarea-item-header">
                    ...
                    <span className={`estado-badge ${tarea.asignacion_estado}`}>
                      {tarea.asignacion_estado === STATUS.TAREA.PENDIENTE && '⏳ Pendiente'}
                      {tarea.asignacion_estado === STATUS.TAREA.EN_PROGRESO && '🔄 En progreso'}
                      {tarea.asignacion_estado === STATUS.TAREA.COMPLETADA && '✅ Completada'}
                    </span>
                  </div>
                  ...
                  <div className="tarea-meta">
                    <div className={`fecha-info ${estadoClase}`}>
                      ...
                      <span>
                        ...
                        {tarea.asignacion_estado !== STATUS.TAREA.COMPLETADA && (
                          <>
                  ...
                  {tarea.asignacion_estado === STATUS.TAREA.COMPLETADA && tarea.asignacion_comentarios && (
                  ...
                  {diasRestantes >= 0 && (
                    puedeResponder ? (
                      <button className="btn-responder" onClick={() => handleResponderClick(tarea)}>
                        ...
                        {tarea.asignacion_estado === STATUS.TAREA.PENDIENTE ? 'Enviar Respuesta' : 'Actualizar Respuesta'}
                      </button>
                    ) : (
